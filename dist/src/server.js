...
webSocket_1.createWebSocket({ server, path: process.env.BASE_PATH || '' });
function serverStart() {
    server
        .listen(utils_1.getConfig().PORT, () => {
        mongoDB_1.connectDB(index_1.BlackBoxApp)
            .then(() => {
            index_1.BlackBoxApp.emit('eventLog', "SERVER_IS_RUNNING", `port: ${utils_1.getConfig().PORT}, mode: ${process.env.NODE_ENV}`);
